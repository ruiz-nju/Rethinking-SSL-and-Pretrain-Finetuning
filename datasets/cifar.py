...
def compute_adjustment_list(label_list, tro, self):
...
class CIFAR102():
    ...
    def get_cifar10(self, root):
        ...
        l_samples = make_imb_data(self.num_max, self.num_classes, self.imb_ratio_label, 1, 0)
        u_samples = make_imb_data(self.num_max_u, self.num_classes, self.imb_ratio_unlabel, 0, self.flag_reverse_LT)

        train_labeled_idxs, train_unlabeled_idxs = train_split(base_dataset.targets, l_samples, u_samples, self)
...
class STL102():
    ...
    def get_stl10(self, root):
        ...
        l_samples = make_imb_data(self.num_max, self.num_classes, self.imb_ratio_label, 1, 0)
        train_labeled_idxs = train_split_l(train_labeled_dataset.labels, l_samples, self)
...
class SMALLIMAGENET():
    def __init__(self):
        pass 
    def get_smallimagenet(self, root):
        assert self.img_size == 32 or self.img_size == 64, 'img size should only be 32 or 64!!!'
        base_dataset = SmallImageNet(root, self.img_size, True)
        ...
        # train_labeled_idxs = train_split(base_dataset.targets, num_labeled_data_per_cls, num_classes, self.seed)
        train_labeled_idxs = train_split_l(base_dataset.targets, num_labeled_data_per_cls, self)

        train_labeled_dataset = SmallImageNet(root, self.img_size, True,
                                            transform=Transform_Con_Labeled(mean=dataset_mean, std=dataset_std,
                                                                            img_size=self.img_size),
                                            indexs=train_labeled_idxs)
        train_unlabeled_dataset = SmallImageNet(root, self.img_size, True,
                                                transform=TransformFixMatch(mean=dataset_mean, std=dataset_std,
                                                                            img_size=self.img_size))
        test_dataset = SmallImageNet(root, self.img_size, False, transform=transform_val)

        arr = np.array(num_labeled_data_per_cls)
        tar_index = np.selfort(-arr)
...
def train_split(labels, n_labeled_per_class, n_unlabeled_per_class, self):
...
def train_split_l(labels, n_labeled_per_class, self):
    labels = np.array(labels)
    train_labeled_idxs = []
    # train_unlabeled_idxs = []
    for i in range(self.num_classes):
