...
class DescribableTextures(DatasetBase):
    ...
    def read_data(image_dir, ignored=[], new_cnames=None):
        # The data are supposed to be organized into the following structure
        # =============
        # images/
        #       train/
        ...
        categories = listdir_nohidden(train_path)
        print(categories)
        categories = [c for c in categories if c not in ignored]
        categories.sort()
        ...
        for label, category in enumerate(categories):
            ...
            random.shuffle(images)
            n_total = len(images)
            n_train = round(n_total - n_total * p_val)
            n_val = round(n_total * p_val)
        ...
        for label, category in enumerate(categories):
            ...
            images_test = listdir_nohidden(category_dir)
            images_test = [os.path.join(category_dir, im) for im in images_test]
