...
from .datasets import build_dataset, build_dataset_out
...
def build_data_loader(
    ...
    data_loader = torch.utils.data.DataLoader(
        ...
        drop_last=is_train
        and len(data_source)
        >= batch_size,  # 通常在训练时，为了保证每个 batch 的大小一致，可以将 drop_last 设置为 True；而在验证或测试时，为了使用所有样本，通常将其设置为 False
...
class DataManager:
    ...
    def __init__(
        ...
        # Load dataset
        if not outdataset:
            print("* Loading OOD datasetttttttttttttttttt")
            dataset = build_dataset_out(cfg)
        else:
        ...
        if (
            dataset.train_x is not None
            and dataset.val is not None
            and dataset.test is not None
        ):
            all_data = dataset.train_x + dataset.val + dataset.test
            print(f"* All data size: {len(all_data)}")
            all_loader = build_data_loader(
                cfg,
                sampler_type=cfg.DATALOADER.TEST.SAMPLER,
