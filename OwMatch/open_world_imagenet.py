...
class ImageNetDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # check if the image is a valid one
        target = self.targets[index]
        sample = self.loader(path)
        if sample is None:
            raise ValueError(f'Image at {path} is missing or unreadable.')
        ...
        if sample is None:
            ...
            raise ValueError(f'Image at {path} is missing or unreadable.')
        if target is None:
            raise ValueError(f'Target at {path} is missing or unreadable.')
        if index is None:
            raise ValueError(f'Index at {path} is missing or unreadable.')
